...
module.exports = async ({graphql, actions}) => {
  ...
  const newestBlogEntry = await graphql(graphqlString('/blog/'));
  const newestBlogNode = newestBlogEntry.data.allMarkdownRemark.edges[0].node;

  const newestRiscvEntry = await graphql(graphqlString('/riscv/'));
  const newestRiscvNode = newestRiscvEntry.data.allMarkdownRemark.edges[0].node;
  ...
  ['/riscv/', '/riscv'].map(slug => {
    createRedirect({
      fromPath: slug,
      redirectInBrowser: true,
      toPath: newestRiscvNode.fields.slug,
    });
  });
